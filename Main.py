...
def main():
    ...
    while opc != ord("4"):
        ...
        screen.addstr(
            4, (x - 40), "IP Node: " + str(node.ipAddrNode), curses.color_pair(2)
        )
        screen.addstr(
            5, (x - 40), "ID Successor: " + str(node.idSuccessor), curses.color_pair(2)
        )
        screen.addstr(
            6,
            (x - 40),
            "IP Successor: " + str(node.ipAddrSuccessor),
            curses.color_pair(2),
        )
        screen.addstr(
            7,
            (x - 40),
            "ID Predecessor: " + str(node.idPredecessor),
            curses.color_pair(2),
        )
        screen.addstr(
            8,
            (x - 40),
            "IP Predecessor: " + str(node.ipAddrPredecessor),
            curses.color_pair(2),
        )
        screen.addstr(9, (x - 40), "----------------------------------")
        ###
        screen.addstr(
            10, 40, "###    PyChord   -   P2P Node    ###", curses.color_pair(1)
        )
        ...
        if opc == ord("1"):
            ...
            if node.idSuccessor == None and node.idPredecessor == None:
                ...
                screen.addstr(
                    15,
                    40,
                    "Ring Initialized with node: " + str(node.code),
                    curses.color_pair(4),
                )
                screen.refresh()
                res = screen.getch()
                ...
                screen.border(0)
                screen.addstr(
                    15, 40, "Ring can not be initialized...", curses.color_pair(3)
                )
                screen.refresh()
                res = screen.getch()
                curses.endwin()
        ...
        if opc == ord("2"):
            if node.ipAddrPredecessor and node.ipAddrSuccessor:
                ...
                screen.addstr(
                    15,
                    40,
                    "LEAVE to Sucessor: " + str(rmsg_sucessor["dest_ip_addr"]),
                    curses.color_pair(4),
                )
                screen.addstr(
                    20,
                    40,
                    "LEAVE to Predecessor:" + str(rmsg_predecessor["dest_ip_addr"]),
                    curses.color_pair(4),
                )
                screen.refresh()
                ...
                screen.addstr(
                    15,
                    40,
                    "IP ADDRESS OF SUCESSOR AND PREDECESSOR IS NULL",
                    curses.color_pair(3),
                )
                screen.refresh()
                res = screen.getch()
